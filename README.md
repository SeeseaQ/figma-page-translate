...
| 场景 | 痛点 | Skill 怎么帮你 |
| --- | --- | --- |
| **电商出海** | Alibaba / Made-in-China / 独立站的产品详情页要同时上架英、阿、西、俄、土 5 种语言，设计师做完一版源稿后全靠复制页面手改 | 一句「把这页翻译成阿拉伯语」→ 扫描 → 翻译 → 原位写回 → 复扫。一个语种从一天缩到几分钟 |
| **多语言产品图册** | 一本 20 页产品图册做 6 种语言版本，型号 / 参数表 / 尺寸标注必须原样保留，只有描述性文字翻 | 自动分类「该翻 / 该保留」，型号用正则识别后跳过，字号 / 字重 / 对齐全部保留 |
| **多语言说明书** | 产品的安装 / 操作 / 维护说明书在 Figma 排版，阿语 RTL 和俄语西里尔字体设计师没法手动搞 | RTL LEFT↔RIGHT 自动翻转，目标字体 + 字重映射自动填充，写完自动复扫确认残留为 0 |
| **留学作品集** | 作品集 / 设计提案在 Figma 排版后需要中英双语或多语版本用于升学、求职，手动复制改字排版容易跑偏 | 一句指令翻译整页，字体 / 字号 / 字重 / 对齐原样保留，MiSans VF 覆盖中英日韩等主流语言开箱即用 |
...
任何「源语言 → 目标语言」组合。默认字体 **MiSans VF** 已覆盖拉丁、中日韩、西里尔等主流语言，无需更换；只有阿拉伯 / 希伯来 / 波斯等 RTL 脚本需换成 Figma 内置的 Noto Sans Arabic。

| 目标语言 | 方向 | 字体 | 对齐处理 |
| --- | --- | --- | --- |
| 英语 / 德语 / 意大利语 / 葡萄牙语 | LTR | MiSans VF（原样保留） | 保持原对齐 |
| 法语 / 西班牙语 / 土耳其语 | LTR | MiSans VF（原样保留） | 保持原对齐 |
| 俄语 / 乌克兰语 | LTR | MiSans VF（含西里尔字形） | 保持原对齐 |
| 中文 / 日文 / 韩文 | LTR | MiSans VF（含 CJK 字形） | 保持原对齐 |
| 阿拉伯语 / 希伯来语 / 波斯语 | RTL | Noto Sans Arabic（Figma 内置） | `LEFT ↔ RIGHT` 翻转，`CENTER` / `JUSTIFIED` 不变 |
